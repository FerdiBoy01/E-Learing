...
// ENDPOINTS TRANSAKSI
// ==========================================
// 🔥 1. [STUDENT] Beli Kelas Pakai Poin (FULL KELAS)
router.post("/pay-with-points", protect, transactionController.payWithPoints);

// 🔥 1.5 [STUDENT] Beli Materi Eceran Pakai Poin (MICROTRANSACTION)
router.post(
  "/unlock-material",
  protect,
  transactionController.unlockMaterialWithPoints,
);
...
router.get("/pending", protect, async (req, res) => {
  try {
    ...
    const transactions =
      await transactionService.getPendingTransactions(lecturerId);
