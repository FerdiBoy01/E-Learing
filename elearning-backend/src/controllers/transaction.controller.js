...
const payWithPoints = async (req, res, next) => {
  try {
    ...
    if (course.type !== 'PROJECT_BASED' || course.price === 0) {
      throw new AppError("Kelas ini gratis coy, mending langsung enroll aja!", 400);
    }

    // Cek saldo Poin Mahasiswa
    const student = await prisma.user.findUnique({ where: { id: studentId } });
    if (student.points < course.price) {
      throw new AppError(`Poin lu kurang coy! Butuh ${course.price} Poin, saldo lu cuma ${student.points} Poin.`, 400);
    }

    // Cek apakah mahasiswa udah punya kelasnya
    const existingEnrollment = await prisma.enrollment.findUnique({
      where: { student_id_course_id: { student_id: studentId, course_id: course_id } }
    });
    if (existingEnrollment) throw new AppError("Lu udah punya kelas ini coy!", 400);
...
const manualPayment = async (req, res, next) => {
  try {
    ...
    if (!receiptUrl) throw new AppError("Bukti transfer wajib disertakan!", 400);
    ...
    return sendSuccess(res, 201, "Bukti transfer berhasil dikirim. Menunggu validasi Instruktur.", { transaction });
  } catch (error) {
    next(error);
  }
};
