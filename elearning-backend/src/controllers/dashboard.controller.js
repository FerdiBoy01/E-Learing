...
const getLecturerDashboard = async (req, res, next) => {
  try {
    ...
    return sendSuccess(res, 200, 'Statistik Dashboard Dosen berhasil diambil', stats);
  } catch (error) {
    next(error);
  }
};
...
const getStudentDashboard = async (req, res, next) => {
  try {
    ...
    return sendSuccess(res, 200, 'Statistik Dashboard Mahasiswa berhasil diambil', stats);
  } catch (error) {
    next(error);
  }
};
