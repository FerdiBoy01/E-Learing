...
const createNewCourse = async (data, user) => {
  ...
  if (data.visibility === 'PRIVATE') {
    // Jika dosen bikin kelas privat tapi gak ngasih kode, kita generate otomatis (6 karakter acak)
    accessCode = data.access_code || crypto.randomBytes(3).toString('hex').toUpperCase();
  }
  ...
  const newCourseData = {
    ...
    lecturer_id: user.id,
    is_published: data.is_published || false, // Default selalu draft saat pertama dibuat
  };
...
module.exports = { 
  createNewCourse, getCourseCatalog, getCourseDetail, updateExistingCourse, deleteExistingCourse 
};