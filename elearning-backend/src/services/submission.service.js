...
const submitChallenge = async (studentId, materialId, submissionData) => {
  ...
  if (material.type !== 'CHALLENGE') {
    throw new AppError('Anda hanya bisa melakukan submission pada materi bertipe CHALLENGE', 400);
  }

  // 2. Lakukan submission
  return await submissionRepository.createOrUpdateSubmission(studentId, materialId, submissionData);
};
